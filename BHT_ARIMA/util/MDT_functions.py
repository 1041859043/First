...
def unfold(tensor,n):
    ...
    re=np.reshape(np.transpose(tensor, pmt),[I, J])
    return re
...
def fold(matrix,n,size_t_ori):
    ...
    pmt = np.append(pmt,range(n+1,N)).astype(np.int)
    matrix=np.reshape(matrix,size_t_pmt)
    re=np.transpose(matrix,pmt)

    return re

def make_duplication_matrix(T,tau):
    H = hankel(range(tau),range(tau-1,T))
    T2= np.prod(H.shape)          #矩阵连乘
    h = np.reshape(H,[1,T2])
...
def tmult(tensor,matrix,n):
    ...
    matrix2=np.matmul(matrix, unfold(tensor, n))
    re=fold(matrix2,n,size)
    return re
