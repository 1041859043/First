...
def fit_ar(Y, p=10):
    r = autocorr(Y, p)
    #print("auto-corr:",r)
    R = linalg.toeplitz(r[:p]) #构造托普利茨矩阵 对角线元素相同
    r = r[1:]
    A = linalg.pinv(R).dot(r)#原理？？
    return A
...
def fit_ar_ma(Y,p=10,q=1):
    ...
    if q>0:
        Res = []
        for i in range(p,N):
            res = Y[i] - np.sum([ a * Y[i-j] for a, j in zip(A, range(1, p+1))], axis=0)  # resi=Xi-a1X(i-1)-a2X(i-2)-a3X(i-3)
            Res.append(res)
        #Res = np.array(Res)
        B = fit_ar(Res, q)
    return A, B
